    ...
    public class SAMAnalyticalUpdateConstructionsByPanelType : GH_SAMVariableOutputParameterComponent
    {
        ...
        public SAMAnalyticalUpdateConstructionsByPanelType()
          : base("SAMAnalytical.UpdateConstructionsByPanelType",
                ...
                "Automatically assign Constructions and ApertureConstructions to Panels by PanelType using a ConstructionManager. The original is not modified; a copy is returned.",
                "SAM",
                "Analytical04")
        {
        }
        ...
        protected override GH_SAMParam[] Inputs
        {
            get
            {
                ...
                result.Add(new GH_SAMParam(new GooConstructionManagerParam() { Name = "constructionManager_", NickName = "constructionManager_", Description = "SAM Analytical ConstructionManager", Access = GH_ParamAccess.item, Optional = true }, ParamVisibility.Binding));
                ...
                for (int i = 0; i < panels.Count; i++)
                {
                    ...
                    if (apertures != null && apertures.Count != 0)
                    {
                        for (int j = 0; j < apertures.Count; j++)
                        {
                            ...
                            List<IMaterial> materials_Temp = constructionManager.GetMaterials<IMaterial>(apertureConstruction_New);
                            if (materials_Temp != null)
                            {
                                foreach (IMaterial material in materials_Temp)
                                {
