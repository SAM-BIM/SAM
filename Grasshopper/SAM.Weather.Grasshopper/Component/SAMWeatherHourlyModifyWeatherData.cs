    ...
    public class SAMWeatherHourlyModifyWeatherData : GH_SAMVariableOutputParameterComponent
    {
        ...
        protected override GH_SAMParam[] Inputs
        {
            get
            {
                ...
                result.Add(new GH_SAMParam(new GooLocationParam { Name = "location_", NickName = "location_", Description = "Optional Location input. The current implementation reads this input but does not apply it to the returned WeatherData.", Access = GH_ParamAccess.item, Optional = true }, ParamVisibility.Binding));
                ...
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_String { Name = "weatherDataTypes_", NickName = "weatherDataTypes_", Description = "Weather variable names to set (e.g. 'DryBulbTemperature', 'RelativeHumidity'). hoursOfYear_ and values_ branches are reused cyclically, in order, if there are fewer branches than weather data types.", Access = GH_ParamAccess.list, Optional = true }, ParamVisibility.Binding));
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Integer { Name = "hoursOfYear_", NickName = "hoursOfYear_", Description = "Tree of hour-of-year indices [0–8759]. Each branch corresponds to one weather data type, reused cyclically if there are fewer branches than weather data types.", Access = GH_ParamAccess.tree, Optional = true }, ParamVisibility.Binding));
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Number { Name = "values_", NickName = "values_", Description = "Tree of values to assign. Each branch corresponds to one weather data type, reused cyclically if there are fewer branches than weather data types. Within each branch, values are reused cyclically, in order, if there are fewer values than hoursOfYear_ entries in that branch.", Access = GH_ParamAccess.tree, Optional = true }, ParamVisibility.Binding));
