    ...
    public class GeometryGetShellsByPoint3D : GH_SAMVariableOutputParameterComponent
    {
        ...
        public GeometryGetShellsByPoint3D()
          : base("Geometry.GetShellsByPoint3D", "Geometry.GetShellsByPoint3D",
              "Partition shells into those that bound a single test point from the remainder",
              "SAM", "Geometry")
        {
        }
        ...
        protected override GH_SAMParam[] Inputs
        {
            get
            {
                ...
                gerenricObject = new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "_shells", NickName = "_shells", Description = "Shells to test against the point", Access = GH_ParamAccess.list };
                gerenricObject.DataMapping = GH_DataMapping.Flatten;
                result.Add(new GH_SAMParam(gerenricObject, ParamVisibility.Binding));

                gerenricObject = new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "_point", NickName = "_Point", Description = "Test point for containment check", Access = GH_ParamAccess.item };
                result.Add(new GH_SAMParam(gerenricObject, ParamVisibility.Binding));
        ...
        protected override GH_SAMParam[] Outputs
        {
            ...
            {
                List<GH_SAMParam> result = new List<GH_SAMParam>();
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "in", NickName = "in", Description = "Shells that contain the test point", Access = GH_ParamAccess.list }, ParamVisibility.Binding));
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "out", NickName = "out", Description = "Shells that do not contain the test point", Access = GH_ParamAccess.list }, ParamVisibility.Binding));
                return result.ToArray();
            }
        }
