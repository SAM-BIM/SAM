...
namespace SAM.Core
{
    public static partial class Create
    {
        ...
        public static DelimitedFileTable DelimitedFileTable(DelimitedFileType delimitedFileType, string path, int namesIndex, int headerCount, System.Text.Encoding encoding)
        {
            if (delimitedFileType == DelimitedFileType.Undefined || string.IsNullOrEmpty(path) || !System.IO.File.Exists(path))
                return null;

            DelimitedFileTable result = null;
            using (DelimitedFileReader delimitedFileReader = new DelimitedFileReader(delimitedFileType, path, encoding))
                result = new DelimitedFileTable(delimitedFileReader, namesIndex, headerCount);
