...
namespace SAM.Geometry.Object.Spatial
{
    public static partial class Query
    {
        public static void ViewField<T>(this IEnumerable<T> face3DObjects, Vector3D viewDirection, out List<LinkedFace3D> linkedFace3Ds_Hidden, out List<LinkedFace3D> linkedFace3Ds_Visible, bool hidden = true, bool visible = true, double tolerance_Area = Tolerance.MacroDistance, double tolerance_Snap = Tolerance.MacroDistance, double tolerance_Angle = Tolerance.Angle, double tolerance_Distance = Tolerance.Distance) where T : IFace3DObject
        {
            ...
            List<Tuple<LinkedFace3D, Geometry.Planar.BoundingBox2D, Geometry.Planar.Face2D>> tuples = new List<Tuple<LinkedFace3D, Geometry.Planar.BoundingBox2D, Geometry.Planar.Face2D>>();
            ...
            foreach (Tuple<LinkedFace3D, Geometry.Planar.BoundingBox2D, Geometry.Planar.Face2D> tuple_Temp in tuples)
            {
                Geometry.Planar.Face2D face2D = tuple_Temp.Item3;
                ...
                List<Geometry.Planar.IClosed2D> closed2Ds = face2D.InternalEdge2Ds;
                if (closed2Ds != null)
                {
                    foreach (Geometry.Planar.IClosed2D closed2D in closed2Ds)
                    {
                        List<Geometry.Planar.Segment2D> segment2Ds_Temp = (closed2D as Geometry.Planar.ISegmentable2D)?.GetSegments();
                        if (segment2Ds_Temp != null)
                        {
                            segment2Ds.AddRange(segment2Ds_Temp);
                        }
            ...
            List<Geometry.Planar.Face2D> face2Ds = Geometry.Planar.Create.Face2Ds(segment2Ds, EdgeOrientationMethod.Undefined, tolerance_Distance);
            if (face2Ds == null)
            {
                return;
            }

            List<Geometry.Planar.IClosed2D> closed2Ds_Holes = Geometry.Planar.Query.Holes(face2Ds);
            if (closed2Ds_Holes != null && closed2Ds_Holes.Count > 0)
            {
                closed2Ds_Holes.ForEach(x => face2Ds.Add(new Geometry.Planar.Face2D(x)));
            }

            List<Tuple<Geometry.Planar.Face2D, Geometry.Planar.Point2D>> tuples_2D = face2Ds.ConvertAll(x => new Tuple<Geometry.Planar.Face2D, Geometry.Planar.Point2D>(x, x.GetInternalPoint2D(tolerance_Distance)));
            for (int i = 0; i < tuples.Count; i++)
            {
                Tuple<LinkedFace3D, Geometry.Planar.BoundingBox2D, Geometry.Planar.Face2D> tuple = tuples[i];
                ...
                if (vector3D_Project != null && vector3D_Project.IsValid() && vector3D_Project.Length > tolerance_Distance)
                {
                    ...
                    if (angle < tolerance_Angle)
                    {
                        if (hidden)
                        {
                            ...
                            {
                                tuples_Hidden[i] = new Tuple<LinkedFace3D, List<Geometry.Planar.Face2D>>(tuple.Item1, new List<Geometry.Planar.Face2D>());
                            }
                ...
                foreach (Tuple<Geometry.Planar.Face2D, Geometry.Planar.Point2D> tuple_2D in tuples_2D)
                {
                    ...
                    {
                        tuples_Result[i] = new Tuple<LinkedFace3D, List<Geometry.Planar.Face2D>>(tuple.Item1, new List<Geometry.Planar.Face2D>());
                    }

                    tuples_Result[i].Item2.Add(tuple_2D.Item1);
                }
            }
